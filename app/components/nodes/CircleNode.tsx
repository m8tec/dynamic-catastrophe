...
export default function CircleNode({ id, data }: CircleNodeProps) {
  ...
  const borderColor = (isActive || isSelectable) ? COLORS.nodeBackgroundActive : (isDiscovered ? COLORS.nodeBorderInactive : COLORS.nodeBorderUndiscovered);
  const color = isActive ? COLORS.nodeTextActive : (isSelectable ? COLORS.nodeBackgroundActive : isDiscovered ? COLORS.nodeTextInactive : COLORS.nodeTextUndiscovered);

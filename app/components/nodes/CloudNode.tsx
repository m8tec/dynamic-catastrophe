...
export default function CloudNode({ id, data }: CloudNodeProps) {
  ...
  const color = isActive ? COLORS.nodeTextActive : isDiscovered ? COLORS.nodeTextInactive : COLORS.nodeTextUndiscovered;
  const fill = isActive ? COLORS.nodeBackgroundActive : isDiscovered ? COLORS.nodeBackgroundInactive : COLORS.nodeBackgroundUndiscovered;
  const stroke = (isActive || isSelectable) ? COLORS.nodeBackgroundActive : isDiscovered ? COLORS.nodeBorderInactive : COLORS.nodeBorderUndiscovered;
